...
const appendTextRows = async (container, textRows, completeCallbackFn) => {
  ...
  container.appendChild(createSpanEl(content, animTime));
...
window.addEventListener('load', () => {
  const observer = new MutationObserver(() => scrollToBottom(consoleDiv));
  observer.observe(consoleDiv, { childList: true });

  appendTextBlocks(consoleDiv, TEXT_BLOCKS);
});
